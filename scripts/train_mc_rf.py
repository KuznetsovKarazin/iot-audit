...
def main():
    ...
    print(f"[rf-mc] training ({X_train.shape[0]} samples, {X_train.shape[1]} features, classes={len(class_map)})...")
    t0 = time.time()
    model.fit(X_train, y_train)
    print(f"[rf-mc] done in {time.time()-t0:.2f}s")
    ...
    print("[rf-mc] metrics:", json.dumps(metrics, indent=2))
